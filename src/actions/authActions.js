...
export const authGoogle = (formValues) => {
  return async (dispatch, getState) => {
    const response = await datingBase.post("/api/users/googleAuth", {
...
export const registerGoogle = (formValues) => {
  return async (dispatch, getState) => {
    await datingBase
      .post("/api/users/registerGoogle", formValues, {
        headers: {
...
export const editPassword = (formValues) => {
  return async (dispatch, getState) => {
    const res = await datingBase.post(
      "/api/users/changeSettingsPassword",
      formValues
    );
    if (!res.data.message) {
      dispatch({
...
export const editMainInfo = (formValues) => {
  return async (dispatch, getState) => {
    const res = await datingBase.post("/api/users/changeMainInfo", formValues);
    dispatch({
      type: GET_USER,
      payload: res.data,
    });
